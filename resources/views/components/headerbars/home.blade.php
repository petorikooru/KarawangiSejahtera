<header id="header" class="fixed top-0 left-0 w-full flex justify-between items-center text-[#000000] font-semibold px-6 lg:px-10 2xl:px-30 py-4 z-50 transition-all duration-300 ease-in-out">
    ...
    <div class="shrink-0">
        <a href="/">
            <img src="{{ url('/images/png/logo.png') }}" alt="Logo website" class="w-32 sm:w-40 md:w-48">
        </a>
    </div>

    <!-- Center (Navigation) -->
    <nav class="hidden md:flex space-x-12 justify-center flex-grow">
    ...
    <div id="mobileMenu" class="md:hidden hidden a<script>
        window.addEventListener('scroll', function () {
            const header = document.getElementById('header');
            if (window.scrollY > 50) { // Adjust 50 to your preferred scroll threshold
                header.classList.add('bg-[#F5F5F5]'); // Change this to your preferred background color
            } else {
                header.classList.remove('bg-[#F5F5F5]');
            }
        });
    </script>bsolute top-16 left-0 w-full bg-white shadow-md z-40 p-4 space-y-4">
